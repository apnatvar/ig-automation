...
const TOKEN = process.env.TOKEN || "meatyhamhock";
...
function verifyXHubSignature(
  ...
  const expected =
    "sha1=" +
    createHmac("sha1", APP_SECRET).update(rawBody, "utf8").digest("hex");
...
export async function POST(
  ...
  if (platform === "facebook") {
    const signature =
      ...
      req.headers.get("x-hub-signature-256"); // prefer sha1 if both set
